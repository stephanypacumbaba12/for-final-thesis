...
class Letter_AgeFive : AppCompatActivity() {
        ...
        userRef.addValueEventListener(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                if (snapshot.exists()) {
                    ...
                    Toast.makeText(this@Letter_AgeFive, "User data not found for $subject", Toast.LENGTH_SHORT).show()
                }
            }

            override fun onCancelled(error: DatabaseError) {
                Toast.makeText(this@Letter_AgeFive, "Failed to fetch data", Toast.LENGTH_SHORT).show()
            }
        })
    ...
    private fun navigateToQuiz(level: Int, username: String?, subject: String?) {
        val intent = Intent(this, QuestionLetter_5::class.java)
