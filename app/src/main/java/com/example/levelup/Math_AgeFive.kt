...
class Math_AgeFive : AppCompatActivity() {
        ...
        userRef.addValueEventListener(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                if (snapshot.exists()) {
                    ...
                    Toast.makeText(this@Math_AgeFive, "User data not found for $subject", Toast.LENGTH_SHORT).show()
                }
            }

            override fun onCancelled(error: DatabaseError) {
                Toast.makeText(this@Math_AgeFive, "Failed to fetch data", Toast.LENGTH_SHORT).show()
            }
        })
    ...
    private fun navigateToQuiz(level: Int, username: String?, subject: String?) {
        val intent = Intent(this, QuestionMath_5::class.java)
