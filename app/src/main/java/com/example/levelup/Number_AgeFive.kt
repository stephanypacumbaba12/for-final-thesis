...
class Number_AgeFive : AppCompatActivity() {

    private val selectedItems = arrayListOf<Int>()
    private var valueEventListener: ValueEventListener? = null // Store listener to remove later
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val levelHome = findViewById<GifImageView>(R.id.level_home)
        ...
        userRef.addValueEventListener(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                if (snapshot.exists()) {
                    ...
                    Toast.makeText(this@Number_AgeFive, "User data not found for $subject", Toast.LENGTH_SHORT).show()
                }
            }

            override fun onCancelled(error: DatabaseError) {
                Toast.makeText(this@Number_AgeFive, "Failed to fetch data", Toast.LENGTH_SHORT).show()
            }
        })

        levelHome.setOnClickListener {
            val homeIntent = Intent(this, Home5::class.java)
            homeIntent.putExtra("username", username)
            homeIntent.putIntegerArrayListExtra("selectedItems", ArrayList(selectedItems))
    ...
    private fun navigateToQuiz(level: Int, username: String?, subject: String?) {
        val intent = Intent(this, QuestionNumber_5::class.java)
