...
class QuestionColor_4 : AppCompatActivity() {

    private var lastFeedbackIndex = -1
    private lateinit var levelData: QuestionAnswer_color4.LevelData
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        subject = intent.getStringExtra("subject") ?: "Colors"
        currentLevel = intent.getIntExtra("level", 1)

        Log.d("QuestionColor", "Current Level: $currentLevel")

        database = FirebaseDatabase.getInstance().reference

        levelData = QuestionAnswer_color4.getLevelData(currentLevel)
            ?: run {
                Log.e("QuestionColor", "Level data for level $currentLevel not found")
    ...
    private fun loadQuestion() {
        if (currentQuestionIndex < levelData.questions.size) {
            ...
            Log.d("QuestionColor", "Loading question at index: $currentQuestionIndex")
            Log.d("QuestionColor", "Question Text: $questionText")
            Log.d("QuestionColor", "Choices: ${choices.joinToString(", ")}")
