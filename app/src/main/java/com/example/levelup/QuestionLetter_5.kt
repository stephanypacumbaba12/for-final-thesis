...
class QuestionLetter_5 : AppCompatActivity() {

    private var lastFeedbackIndex = -1
    private lateinit var levelData: QuestionAnswer_letter5.LevelData
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        subject = intent.getStringExtra("subject") ?: "Letters"
        currentLevel = intent.getIntExtra("level", 1)

        Log.d("QuestionLetter", "Current Level: $currentLevel")

        database = FirebaseDatabase.getInstance().reference

        levelData = QuestionAnswer_letter5.getLevelData(currentLevel)
            ?: run {
                Log.e("QuestionLetter", "Level data for level $currentLevel not found")
    ...
    private fun loadQuestion() {
        if (currentQuestionIndex < levelData.questions.size) {
            ...
            val questionText = levelData.questionTexts[currentQuestionIndex]  // Ensure letter-based question is displayed
            val choices = levelData.choices[currentQuestionIndex]

            Log.d("QuestionLetter", "Loading question at index: $currentQuestionIndex")
            Log.d("QuestionLetter", "Question Text: $questionText")
            Log.d("QuestionLetter", "Choices: ${choices.joinToString(", ")}")
