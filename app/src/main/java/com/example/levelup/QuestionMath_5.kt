...
class QuestionMath_5 : AppCompatActivity() {

    private var lastFeedbackIndex = -1
    private lateinit var levelData: QuestionAnswer_mathematics5.LevelData
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        subject = intent.getStringExtra("subject") ?: "Mathematics"
        currentLevel = intent.getIntExtra("level", 1)

        Log.d("QuestionMathematics", "Current Level: $currentLevel")

        database = FirebaseDatabase.getInstance().reference

        levelData = QuestionAnswer_mathematics5.getLevelData(currentLevel)
            ?: run {
                Log.e("QuestionMathematics", "Level data for level $currentLevel not found")
    ...
    private fun loadQuestion() {
        if (currentQuestionIndex < levelData.questions.size) {
            ...
            Log.d("QuestionMathematics", "Loading question at index: $currentQuestionIndex")
            Log.d("QuestionMathematics", "Question Text: $questionText")
            Log.d("QuestionMathematics", "Choices: ${choices.joinToString(", ")}")
