...
class QuestionNumber_4 : AppCompatActivity() {

    private var lastFeedbackIndex = -1
    private lateinit var levelData: QuestionAnswer_number4.LevelData
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        subject = intent.getStringExtra("subject") ?: "Number"
        currentLevel = intent.getIntExtra("level", 1)

        Log.d("QuestionNumber", "Current Level: $currentLevel")

        database = FirebaseDatabase.getInstance().reference

        levelData = QuestionAnswer_number4.getLevelData(currentLevel)
            ?: run {
                Log.e("QuestionNumber", "Level data for level $currentLevel not found")
    ...
    private fun loadQuestion() {
        if (currentQuestionIndex < levelData.questions.size) {
            ...
            Log.d("QuestionNumber", "Loading question at index: $currentQuestionIndex")
            Log.d("QuestionNumber", "Question Text: $questionText")
            Log.d("QuestionNumber", "Choices: ${choices.joinToString(", ")}")
    ...
    private fun saveScoreToFirebase() {
        ...
        subjectRef.child("attempt").get().addOnSuccessListener { snapshot ->
            val currentAttempt =
                snapshot.getValue(Int::class.java) ?: 0 // Default to 0 if no value exists
