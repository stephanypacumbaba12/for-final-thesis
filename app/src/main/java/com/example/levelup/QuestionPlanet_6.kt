...
class QuestionPlanet_6 : AppCompatActivity() {

    private var lastFeedbackIndex = -1
    private lateinit var levelData: QuestionAnswer_planet6.LevelData
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        subject = intent.getStringExtra("subject") ?: "Planet"
        currentLevel = intent.getIntExtra("level", 1)

        Log.d("QuestionPlanet", "Current Level: $currentLevel")

        database = FirebaseDatabase.getInstance().reference

        levelData = QuestionAnswer_planet6.getLevelData(currentLevel)
            ?: run {
                Log.e("QuestionPlanet", "Level data for level $currentLevel not found")
    ...
    private fun loadQuestion() {
        if (currentQuestionIndex < levelData.questions.size) {
            ...
            Log.d("QuestionPlanet", "Loading question at index: $currentQuestionIndex")
            Log.d("QuestionPlanet", "Question Text: $questionText")
            Log.d("QuestionPlanet", "Choices: ${choices.joinToString(", ")}")
