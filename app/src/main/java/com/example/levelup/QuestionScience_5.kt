...
class QuestionScience_5 : AppCompatActivity() {

    private var lastFeedbackIndex = -1
    private lateinit var levelData: QuestionAnswer_science5.LevelData
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        subject = intent.getStringExtra("subject") ?: "Science"
        currentLevel = intent.getIntExtra("level", 1)

        Log.d("QuestionScience", "Current Level: $currentLevel")

        database = FirebaseDatabase.getInstance().reference

        levelData = QuestionAnswer_science5.getLevelData(currentLevel)
            ?: run {
                Log.e("QuestionScience", "Level data for level $currentLevel not found")
    ...
    private fun loadQuestion() {
        if (currentQuestionIndex < levelData.questions.size) {
            ...
            Log.d("QuestionScience", "Loading question at index: $currentQuestionIndex")
            Log.d("QuestionScience", "Question Text: $questionText")
            Log.d("QuestionScience", "Choices: ${choices.joinToString(", ")}")
