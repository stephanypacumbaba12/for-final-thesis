...
class QuestionShape_5 : AppCompatActivity() {

    private var lastFeedbackIndex = -1
    private lateinit var levelData: QuestionAnswer_shape5.LevelData
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        subject = intent.getStringExtra("subject") ?: "Shapes"
        currentLevel = intent.getIntExtra("level", 1)

        Log.d("QuestionShape", "Current Level: $currentLevel")

        database = FirebaseDatabase.getInstance().reference

        levelData = QuestionAnswer_shape5.getLevelData(currentLevel)
            ?: run {
                Log.e("QuestionShape", "Level data for level $currentLevel not found")
    ...
    private fun loadQuestion() {
        if (currentQuestionIndex < levelData.questions.size) {
            ...
            Log.d("QuestionShape", "Loading question at index: $currentQuestionIndex")
            Log.d("QuestionShape", "Question Text: $questionText")
            Log.d("QuestionShape", "Choices: ${choices.joinToString(", ")}")
