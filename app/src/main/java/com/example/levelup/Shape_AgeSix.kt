...
class Shape_AgeSix : AppCompatActivity() {
        ...
        userRef.addValueEventListener(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                if (snapshot.exists()) {
                    ...
                    Toast.makeText(this@Shape_AgeSix, "User data not found for $subject", Toast.LENGTH_SHORT).show()
                }
            }

            override fun onCancelled(error: DatabaseError) {
                Toast.makeText(this@Shape_AgeSix, "Failed to fetch data", Toast.LENGTH_SHORT).show()
            }
        })
    ...
    private fun navigateToQuiz(level: Int, username: String?, subject: String?) {
        val intent = Intent(this, QuestionShape_6::class.java)
