...
class Vegetable_AgeFour :AppCompatActivity() {
        ...
        userRef.addValueEventListener(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                if (snapshot.exists()) {
                    ...
                    Toast.makeText(this@Vegetable_AgeFour, "User data not found for $subject", Toast.LENGTH_SHORT).show()
                }
            }

            override fun onCancelled(error: DatabaseError) {
                Toast.makeText(this@Vegetable_AgeFour, "Failed to fetch data", Toast.LENGTH_SHORT).show()
            }
        })
    ...
    private fun navigateToQuiz(level: Int, username: String?, subject: String?) {
        val intent = Intent(this, QuestionVegetable_4::class.java)
